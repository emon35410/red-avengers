...
const FindDonor = () => {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 dark:bg-[#060910]">
        <Loader2 className="w-8 h-8 text-rose-600 animate-spin" />
        <p className="mt-4 font-heading text-[10px] font-black uppercase tracking-widest text-rose-600">Searching DB...</p>
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12">
          <div>
            ...
            <p className="mt-4 text-slate-500 dark:text-slate-400 text-sm">
              Showing <span className="text-rose-500 font-bold">{filteredDonors.length}</span> heroes ready to help.
            </p>
          </div>
        ...
        {filteredDonors.length === 0 ? (
          <div className="text-center py-20 bg-white dark:bg-[#0d1117] rounded-3xl border border-dashed border-slate-200 dark:border-white/10">
            <p className="text-slate-500 dark:text-slate-400">No donors found in this sector.</p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div className="overflow-hidden">
                    <h3 className="text-lg font-black text-slate-900 dark:text-white truncate tracking-tight">{donor.name}</h3>
                ...
                <div className="space-y-3 mb-6">
                  <div className="flex items-center gap-3 text-slate-600 dark:text-slate-400 text-xs">
                    <div className="p-2 bg-slate-50 dark:bg-white/5 rounded-lg"><MapPin size={14} /></div>
                    <span>{donor.upazila}, {donor.district}</span>
                  </div>
                  <div className="flex items-center gap-3 text-slate-600 dark:text-slate-400 text-xs">
                    <div className="p-2 bg-slate-50 dark:bg-white/5 rounded-lg"><Phone size={14} /></div>
