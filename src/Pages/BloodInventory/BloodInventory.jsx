...
const BLOOD_COLORS = {
  ...
  "AB+": "from-purple-500 to-rose-600",
  "AB-": "from-purple-400 to-purple-600",
  "O+": "from-red-600 to-rose-700",
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-zinc-950 p-6 md:p-12 transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-12">
          <h1 className="text-4xl font-black italic tracking-tighter dark:text-white">
            PUBLIC <span className="text-rose-600">INVENTORY</span>
          </h1>
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-2">
            Verified Blood Stock Status
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4 mb-16">
          {summary.map((item) => (
            <div
              key={item._id}
              className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 p-5 rounded-3xl text-center"
            >
              <div
                className={`w-10 h-10 mx-auto rounded-xl bg-linear-to-br ${BLOOD_COLORS[item._id]} flex items-center justify-center mb-3 shadow-lg`}
              >
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-8 py-5">
                        <span
                          className={`px-3 py-1.5 rounded-lg bg-linear-to-br ${BLOOD_COLORS[log.bloodGroup]} text-white font-bold text-xs`}
                        >
                          {log.bloodGroup}
                        </span>
                      </td>
                      <td className="px-8 py-5 font-bold text-sm dark:text-slate-200">
                        {log.volume || "450 ml"}
                      </td>
                      <td className="px-8 py-5">
                        <span className="text-[10px] font-black uppercase text-emerald-500 bg-emerald-500/10 px-2 py-1 rounded">
                          ✓ {log.quality || "Verified"}
                        </span>
                      </td>
                      <td className="px-8 py-5">
                        <span
                          className={`text-xs font-bold ${daysLeft < 10 ? "text-rose-500" : "text-emerald-500"}`}
                        >
